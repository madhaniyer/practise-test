            ...
            {mockTests.map((test) => (
              <div key={test.id} className="rounded-3xl border border-slate-100 bg-slate-50/80 p-4">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className={`rounded-full px-3 py-1 text-sm font-medium ${test.released ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-700"}`}>
                    {test.released ? "Released" : "Locked"}
                  </span>
                </div>
              </div>
            ))}
